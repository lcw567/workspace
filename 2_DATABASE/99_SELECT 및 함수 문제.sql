...
--14. EMPLOYEE테이블에서 메일주소 '_'의 앞이 4자이면서 DEPT_CODE가 D9 또는 D6이고고용일이 90/01/01 ~ 00/12/01이고,
...
SELECT EMP_NAME AS 사원명, RPAD(SUBSTR(EMP_NO,1 ,7), 14, '*') AS 주민등록번호
FROM EMPLOYEE;
...
SELECT EMP_NAME,
    ...
    --ROUND(HIRE_DATE-SYSDATE) * -1 AS "근무일수1",
...
--★★★★
SELECT EMP_NAME, TO_CHAR(SALARY,'L99,999,999') AS "급여조회"
FROM EMPLOYEE;
...
SELECT EMP_NAME,
    ...
    SUBSTR(EMP_NO, 1,6)+ SUBSTR(EMP_NO, 8,14) AS "앞자리 뒷자리의 합"
FROM EMPLOYEE
WHERE EMP_ID = '201';
