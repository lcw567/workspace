...
public class Baekjun {
    public static void main(String[] args) {
        ...
        System.out.println("현재 시각(HH:MM) :");
        String alarmTime = sc.next(); // 알람 시각을 "HH:mm" 형식으로 입력 받음
        
        // 입력받은 시간을 시간과 분으로 분리
        int hour = Integer.parseInt(alarmTime.substring(0, 2));
        int minute = Integer.parseInt(alarmTime.substring(3));
        
        // 창영이의 방법에 따라 알람을 45분 앞으로 설정
        if (minute >= 45) {
            minute -= 45;
        } else {
            minute = 60 - (45 - minute);
