...
//		MultipartRequest multiRequest = MultipartRequest(request, savePath, maxSize, "UTF-8", new DefaultFileRenamePolicy());
		...
		if(JakartaServletFileUpload.isMultipartContent(request)) {
			...
			int requsetMaxSize = 1024 * 1024 * 20; //20mb
			
			//2. 전달된 파일을 저장시킬 폴더경로 가져오기
			String savePath = getServletContext().getRealPath("/resources/board_upfile/");
//			System.out.println(savePath);
			...
			upload.setSizeMax(requsetMaxSize);
			...
			for(FileItem item : formItems) {
				...
				if(item.isFormField()) { //일반파라미터
					...
					String originName = item.getName(); //업로드 요청한 파일명
					if(originName.length() > 0) { //파일을 업로드 했을 떄
						//고유한 파일명 생성
						...
						at.setFilePath("resources/board_upfile/"); // resource 앞에 '/'가 안붙는 이유는 contextPath에 '/'가 붙어있기 때문
					}
				}
			}
