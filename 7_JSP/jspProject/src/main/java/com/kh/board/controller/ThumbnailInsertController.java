...
public class ThumbnailInsertController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(JakartaServletFileUpload.isMultipartContent(request)) {
			...
			String savePath = getServletContext().getRealPath("/resources/thumbnail_upfile/");
			...
			for(FileItem item : formItems) {
				...
				if(item.isFormField()) { //일반파라미터
					...
					if(originName.length() > 0) { //파일을 업로드 했을 떄
						...
						at.setFilePath("resources/thumbnail_upfile/"); // resource 앞에 '/'가 안붙는 이유는 contextPath에 '/'가 붙어있기 때문
